...
export default function Category() {
  ...
  useEffect(() => {
    if (products) {
      ...
      if (options.length > 0) {
        updateProducts = updateProducts.filter((x) =>
          options.includes(x.brand_id)
        );
      }
      updateProducts = updateProducts.filter(
        (x) => x.price >= priceRange.min && x.price <= priceRange.max
      );
                      ...
                      <div className='product-filter' key={x.id}>
                        <label>
                          <input
                            type='checkbox'
                            name={x.name}
                            checked={options.includes(x.id)}
                            value={x.id}
                            onChange={(e) => handleCheckboxChange(x.id, e)}
                          />
                          <span>{x.name}</span>
                        </label>
                      </div>
          ...
          <div className='product-items'>
            {filteredProducts.map((x) => (
              <div className='product-item' key={x.id}>
                ...
                <button className='btn' onClick={() => addToCart(x)}>
                  Sepete Ekle
                </button>
              </div>
            ))}
