        ...
        e.preventDefault();
            ...
            if(userPassword == userPasswords.password1){
                if (userPasswords.password2 == userPasswords.password3){
                    const { data, error } = await supabase.auth.updateUser({ password: userPasswords.password2 });
                    console.log(data);
                    alert('Şifre Değişti');
                }else{
                  ...
                  <form onSubmit={changePassword}>
                      <div className='container password-inputs'>
                          <div className='password-input'>
                              <p>Eski Şifre</p>
                              <input type={isVisible1 ? 'text' : 'password'} placeholder='Eski Şifre' name='password1' required />
                              <img className='eye-img' src={isVisible1 ? "/public/password-off.svg" : "/public/password-on.svg"} alt="" onClick={() => { setIsVisible1(!isVisible1) }} />
                          </div>
                          <div className='password-input'>
                              <p>Yeni Şifre</p>
                              <input type={isVisible2 ? 'text' : 'password'} placeholder='Yeni Şifre' name='password2' required />
                              <img className='eye-img' src={isVisible2 ? "/public/password-off.svg" : "/public/password-on.svg"} alt="" onClick={() => { setIsVisible2(!isVisible2) }} />
                          </div>
                          <div className='password-input'>
                              <p>Yeni Şifre</p>
                              <input type={isVisible3 ? 'text' : 'password'} placeholder='Yeni Şifre' name='password3' required />
